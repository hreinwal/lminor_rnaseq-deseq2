...
### Annotated DEG result tables with custom blastp / eggNOG annotation table! ### ----------------------
message("\nAnnotating Lemna Gene IDs in DEG result tables using custom annotation from blastp & eggNOG ...")
f1 = "~/BLASTdb/Lemna.minor_ref.org.blast.db/uniprotFASTA/Lminor.blastp.eNOG.GOanno.csv"
f2 = "S:/data/BLASTdb/Lemna.minor_ref.org.anno/Lminor.blastp.eNOG.GOanno.csv"
if(file.exists(f1)) {
  message("\nLemna annotation file found locally. \nLoading ",f1," into R session ...")
  skip = F
  lemna.anno = read.csv2(f1, header = T)
} else if (file.exists(f2)){
  message("\nLemna annotation file found in ",f2," \nLoading annotation table into R session ...")
  skip = F
  lemna.anno = read.csv2(f2, header = T)
} else {
  message("\nUnable to find any custom Leman annotation file. Continue without annotating DEG result tables")
  skip = T
}

# Import Lemna minor annotation table
if(skip == F){
  anno.col = c("query","prot.ID.TopHit","organism.TopHit","taxid.TopHit","identity.perc.TopHit","Description.eNOG",
               "description.BestDescr","taxid.BestDescr","identity.perc.BestDescr","combinedID")
  for(i in c("deg.ls","degCut.ls","degLfs.ls")){
    tmp = lapply(get(i), function(x){
      tmp = merge(as.data.frame(x), lemna.anno[,anno.col], all.x = T, by.x = 0, by.y = "query")
      row.names(tmp) <- tmp[,1]
      return(tmp[,-1])
    })
    assign(i, tmp)
...
DE.Cor.Venn = function(df.x, df.y, pcut = .05, typ.x = "DEG in HE", typ.y = "DEG in LE", title = "",
  ...
  x = subset(df.x, padj <= pcut & abs(log2FC) >= LFcut.x)[,"Gene"]
  #y = droplevels(subset(df.y[df.y$padj <= pcut, ], abs(log2FC) >= LFcut.y))[,"Gene"]
  y = subset(df.y, padj <= pcut & abs(log2FC) >= LFcut.y)[,"Gene"]
  ...
  X = df.x[df.x$Gene %in% uni,c("Gene","log2FC")]
  Y = df.y[df.y$Gene %in% uni,c("Gene","log2FC")]
